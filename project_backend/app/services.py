...
def _expand_table_roi(roi: Dict[str, float], image_width: int, image_height: int) -> Dict[str, float]:
    pad_x = 8.0 / max(float(image_width), 1.0)
    pad_y = 8.0 / max(float(image_height), 1.0)
...
class TemplateRequestService:
        ...
        with _connect() as conn:
            conn.execute(
                ...
                INSERT INTO template_requests (
                    ...
                    request_mode, status, user_note, admin_note, created_at
                )
            ...
            for page in source_pages:
                ...
                conn.execute(
                    ...
                    INSERT INTO template_request_pages (
                        id, template_request_id, page_number, page_name,
                        sample_image_url, source_file_name, review_status, created_at
                    )
        ...
        if column_values:
            ...
            with _connect() as conn:
                ...
                conn.execute(
                    ...
                    SET {assignments}, reviewed_at = CURRENT_TIMESTAMP
                    WHERE id = ?
                    """,
    ...
    def delete(self, request_id: str) -> Dict[str, Any]:
        ...
        return {
            "id": request_id,
            "deleted": True,
            "converted_template_id": request_row.get("converted_template_version_id"),
            "deleted_records": {
        ...
        with _connect() as conn:
            ...
            if page_row is None:
                ...
                conn.execute(
                    """
                    INSERT INTO template_request_pages (
                        id, template_request_id, page_number, page_name, sample_image_url, source_file_name, review_status, created_at
                    )
            ...
            conn.execute(
                ...
                INSERT INTO requested_fields (
                    id, template_request_page_id, field_name, display_label,
                    data_type, extraction_method,
    ...
    def update_requested_field(self, request_id: str, field_id: str, payload: RequestedFieldUpdate) -> Dict[str, Any]:
        patch = payload.model_dump(exclude_unset=True)
        column_values: Dict[str, Any] = {}
        for key in {"field_name", "display_label", "data_type", "extraction_method", "user_note"}:
            if key in patch:
                value = patch[key]
                if key == "data_type":
        ...
        with _connect() as conn:
            if column_values:
                ...
                conn.execute(
                    ...
                    SET {set_clause}
                    WHERE id = ? AND template_request_page_id IN (
                        SELECT id FROM template_request_pages WHERE template_request_id = ?
                    )
                    """,
            ...
            row = conn.execute(
                ...
                FROM requested_fields rf
                JOIN template_request_pages trp ON trp.id = rf.template_request_page_id
                WHERE rf.id = ? AND trp.template_request_id = ?
                """,
                (field_id, request_id),
            ).fetchone()
        if row is None:
    ...
    def delete_requested_field(self, request_id: str, field_id: str) -> Dict[str, Any]:
        with _connect() as conn:
            ...
            conn.commit()
        return {"id": field_id, "template_request_id": request_id, "deleted": cursor.rowcount > 0}
...
class AdminTemplateService:
    ...
    def create_template(self, payload: TemplateCreate) -> Dict[str, Any]:
        template_group_id = _stub_id("tgrp")
        template_id = _stub_id("tpl")
        template_code_base = _safe_file_token(payload.name or "template") or "template"
        template_code = f"{template_code_base}_{uuid.uuid4().hex[:8]}"
        with _connect() as conn:
            conn.execute(
                """
                ...
                (
                    template_group_id,
                    template_code,
                    payload.document_type or payload.name,
                    payload.document_type,
                    payload.category,
                    payload.description,
                    payload.created_by,
            ...
            conn.execute(
                ...
                INSERT INTO template_versions (
                    ...
                    detection_mode, main_page_number, similarity_threshold, final_confidence_threshold,
        ...
        with _connect() as conn:
            rows = conn.execute(
                ...
                SELECT
                    ...
                    (
                        SELECT COUNT(*)
                        FROM template_pages tp_count
                        WHERE tp_count.template_version_id = tv.id
                    ) AS page_count,
                    tv.similarity_threshold,
                    tv.final_confidence_threshold,
                    tv.layout_weight,
            ...
            page_rows = conn.execute(
                ...
                ORDER BY template_version_id ASC, page_number ASC
                """
            ).fetchall()
    ...
    def confirm_publish_template(self, template_id: str) -> Dict[str, Any]:
        ...
        with _connect() as conn:
            conn.execute(
                "UPDATE template_versions SET status = 'validated', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                (template_id,),
            )
            conn.commit()
    ...
    def update_template(self, template_id: str, payload: TemplateUpdate) -> Dict[str, Any]:
        ...
        if updates:
            ...
            with _connect() as conn:
                ...
                version_updates = [(column, value) for column, value in updates if column in version_columns]
                group_updates = [(column, value) for column, value in updates if column in group_columns]
                if version_updates:
                    version_set = ", ".join(f"{column} = ?" for column, _ in version_updates)
                    conn.execute(
                        f"UPDATE template_versions SET {version_set}, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        [*[value for _, value in version_updates], template_id],
                    )
                if group_updates:
                    group_set = ", ".join(f"{column} = ?" for column, _ in group_updates)
                    conn.execute(
                        f"""
                        UPDATE template_groups
                        SET {group_set}, updated_at = CURRENT_TIMESTAMP
                        WHERE id = (SELECT template_group_id FROM template_versions WHERE id = ?)
                        """,
        ...
        with _connect() as conn:
            ...
            counts = {
                "publish_jobs": conn.execute(
                    "SELECT COUNT(*) AS count FROM publish_jobs WHERE template_version_id = ?",
                    (template_id,),
                ).fetchone()["count"],
                ...
                "extraction_fields": conn.execute(
                    """
                    SELECT COUNT(*) AS count
                    FROM extraction_fields ef
                    JOIN template_pages tp ON tp.id = ef.template_page_id
                ...
                "verification_anchors": conn.execute(
                    """
                    SELECT COUNT(*) AS count
                    FROM verification_anchors va
                    JOIN template_pages tp ON tp.id = va.template_page_id
                ...
                "template_pages": conn.execute(
                    "SELECT COUNT(*) AS count FROM template_pages WHERE template_version_id = ?",
                    (template_id,),
                ).fetchone()["count"],
                "template_versions": 1,
            }
            conn.execute(
                """
                UPDATE template_requests
                SET converted_template_version_id = NULL,
                    reviewed_at = CURRENT_TIMESTAMP
                WHERE converted_template_version_id = ?
                """,
                (template_id,),
            )
        ...
        with _connect() as conn:
            conn.execute(
                ...
                INSERT INTO template_pages (
                    id, template_version_id, page_number, page_name, sample_image_url,
    ...
    def update_template_field(
        ...
        with _connect() as conn:
            extraction_exists = conn.execute(
                ...
                FROM extraction_fields ef
                JOIN template_pages tp ON tp.id = ef.template_page_id
                WHERE ef.id = ? AND tp.template_version_id = ?
                """,
                (field_id, template_id),
            ).fetchone()
            anchor_exists = None if extraction_exists else conn.execute(
                """
                SELECT va.id
                FROM verification_anchors va
                JOIN template_pages tp ON tp.id = va.template_page_id
                WHERE va.id = ? AND tp.template_version_id = ?
                """,
                (field_id, template_id),
            ).fetchone()
            if extraction_exists:
                ...
                direct_columns = {
                    ...
                    "roi_mode": "roi_mode",
                    "expected_content": "expected_content",
                    "required_for_verification": "required",
                ...
                for key, column in direct_columns.items():
                    if key in patch:
                        ...
                        if key == "required_for_verification":
                            value = bool(value)
                ...
                if column_values:
                    ...
                    conn.execute(
                        ...
                        WHERE id = ?
                        """,
                        [*column_values.values(), field_id],
                    )
            elif anchor_exists:
                column_values = {}
                direct_columns = {
                    "template_page_id": "template_page_id",
                    "field_name": "anchor_name",
                    "data_type": "anchor_type",
                    "expected_text": "expected_text",
    ...
    def delete_template_field(self, template_id: str, field_id: str) -> Dict[str, Any]:
        with _connect() as conn:
            conn.execute(
                ...
                WHERE id = ? AND template_page_id IN (
                    SELECT id FROM template_pages WHERE template_version_id = ?
                )
                """,
                (field_id, template_id),
            )
            conn.execute(
                """
                DELETE FROM verification_anchors
                WHERE id = ? AND template_page_id IN (
                    SELECT id FROM template_pages WHERE template_version_id = ?
                )
                """,
        ...
        with _connect() as conn:
            conn.execute(
                ...
                INSERT INTO ignore_regions (
                    id, template_page_id, region_name,
                    roi_x_ratio, roi_y_ratio, roi_width_ratio, roi_height_ratio,
                    reason, created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    ...
    def suggest_base_version_for_request(
        ...
        with _connect() as conn:
            selected_template = conn.execute(
                """
                SELECT
                    tv.id,
                    tg.name,
            ...
            group_id = selected_template["template_group_id"]
            version_rows = [
                _row_to_dict(row)
                for row in conn.execute(
                """
            ...
            reference_rows = [
                ...
                WHERE tv.template_group_id = ?
                  AND tp.layout_signature_json IS NOT NULL
                ORDER BY tp.template_version_id ASC, tp.page_number ASC
                """,
                (group_id,),
                ).fetchall()
            ]
    ...
    def create_version_from_request(self, request_id: str, payload: TemplateVersionCreate) -> Dict[str, Any]:
        ...
        with _connect() as conn:
            base_template_row = conn.execute(
                """
                SELECT tv.*, tg.name AS template_name, tg.document_type, tg.category, tg.description
                FROM template_versions tv
                JOIN template_groups tg ON tg.id = tv.template_group_id
                WHERE tv.id = ?
                """,
            ...
            if not approved_pages:
                raise HTTPException(status_code=409, detail="At least one approved request page is required to create a Template Version")
            ...
            stored_main_page_number = 1 if detection_mode == "main_page" else selected_main_page_number

            group_id = base_template_row["template_group_id"]
            ...
            next_version = int(next_version_row["next_version"] or 1)
            template_id = _stub_id("tpl")
            template_name = (payload.template_name or f"{base_template_row['template_name']} - Version {next_version}").strip()
            ...
            conn.execute(
                ...
                INSERT INTO template_versions (
                    id, template_group_id, version_number, version_name, status,
                    detection_mode, main_page_number, similarity_threshold, final_confidence_threshold,
                    layout_weight, text_anchor_weight, image_anchor_weight, created_from_version_id,
                    created_at, updated_at
                )
                ...
                (
                    ...
                    payload.similarity_threshold,
                    base_template_row.get("final_confidence_threshold") or 0.75,
                    base_template_row.get("layout_weight") or 0.40,
                    base_template_row.get("text_anchor_weight") or 0.30,
                    base_template_row.get("image_anchor_weight") or 0.30,
                    payload.base_template_id,
                ),
            )

            base_extraction_fields = conn.execute(
                """
                ...
                """,
                (payload.base_template_id,),
            ).fetchall() if payload.reuse_roi else []
            base_anchors = conn.execute(
                """
                SELECT va.*, tp.page_number
                FROM verification_anchors va
                JOIN template_pages tp ON tp.id = va.template_page_id
                WHERE tp.template_version_id = ?
                ORDER BY tp.page_number ASC, va.sort_order ASC, va.created_at ASC
            ...
            for page_index, source_page in enumerate(template_pages_source, start=1):
                page_id = _stub_id("tpl_page")
                signature = _generate_layout_signature_for_source(source_page.get("sample_image_url"))
                conn.execute(
                    """
                    INSERT INTO template_pages (
                        id, template_version_id, page_number, page_name, sample_image_url,
                        normalized_image_url, layout_signature_json, created_at, updated_at
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                    """,
                    (
                        ...
                        page_index,
                        f"Page {page_index}",
                        source_page.get("sample_image_url"),
                        source_page.get("sample_image_url"),
                        signature_to_json(signature) if signature else None,
                    ),
                )
                page_id_by_number[page_index] = page_id

            cloned_field_count = 0
            for field in [_row_to_dict(row) for row in base_extraction_fields]:
                target_page_number = 1 if detection_mode == "main_page" else int(field.get("page_number") or 1)
                target_page_id = page_id_by_number.get(target_page_number)
                ...
                conn.execute(
                    ...
                    (
                        ...
                        field["field_name"],
                        field["display_label"],
                        field.get("data_type") or "text",
                        field.get("extraction_method") or "fixed_roi",
                        field["roi_x_ratio"],
                        field["roi_y_ratio"],
                        field["roi_width_ratio"],
                        field["roi_height_ratio"],
                        field.get("roi_mode") or "fix",
                        field.get("expected_content"),
            ...
            cloned_anchor_count = 0
            for anchor in [_row_to_dict(row) for row in base_anchors]:
                target_page_number = 1 if detection_mode == "main_page" else int(anchor.get("page_number") or 1)
                target_page_id = page_id_by_number.get(target_page_number)
                if not target_page_id:
                    continue
                conn.execute(
                    ...
                    (
                        ...
                        target_page_id,
                        anchor["anchor_name"],
                        anchor.get("anchor_type") or "text",
                        anchor["roi_x_ratio"],
                        anchor["roi_y_ratio"],
                        anchor["roi_width_ratio"],
                        anchor["roi_height_ratio"],
                        bool(anchor.get("required", False)),
                        anchor.get("weight", 1.0),
                        anchor.get("expected_text"),
                        anchor.get("match_type"),
                        anchor.get("regex_pattern"),
                        anchor.get("image_category_id"),
                        anchor.get("sort_order", 0),
                    ),
                )
                cloned_anchor_count += 1
        ...
        return {
            ...
            "created_records": {
                "template_versions": 1,
                "template_pages": len(template_pages_source),
                "extraction_fields": cloned_field_count,
                "verification_anchors": cloned_anchor_count,
            },
        }
    ...
    def convert_request_to_template(self, request_id: str, payload: Optional[TemplateVersionCreate] = None) -> Dict[str, Any]:
        ...
        with _connect() as conn:
            ...
            if request_row["status"] == "converted" and request_row.get("converted_template_version_id"):
                return {
                    "id": request_id,
                    "status": "converted",
                    "converted_template_id": request_row["converted_template_version_id"],
                    "template_id": request_row["converted_template_version_id"],
                    "created_records": {},
                }
            ...
            if not approved_pages:
                raise HTTPException(status_code=409, detail="At least one approved request page is required to create a Template")
            ...
            conn.execute(
                """
                INSERT INTO template_groups (
                    id, template_code, name, document_type, category, description, created_at, updated_at
                )
                VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """,
                (
                    group_id,
                    f"{(_safe_file_token(template_name) or 'template')}_{uuid.uuid4().hex[:8]}",
                    request_row.get("document_type") or template_name,
            ...
            conn.execute(
                ...
                INSERT INTO template_versions (
                    id, template_group_id, version_number, version_name, status,
                    detection_mode, main_page_number, similarity_threshold, final_confidence_threshold,
                    layout_weight, text_anchor_weight, image_anchor_weight, created_at, updated_at
                )
                VALUES (?, ?, 1, ?, 'draft', ?, ?, ?, ?, 0.40, 0.30, 0.30, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """,
                (
                    template_id,
                    group_id,
                    ...
                    detection_mode,
                    stored_main_page_number,
                    payload.similarity_threshold if payload else 0.75,
                    0.75,
                ),
            )
            ...
            for page_index, page in enumerate(template_pages_source, start=1):
                page_id = _stub_id("tpl_page")
                page_fields = [field for field in requested_fields if field.get("template_request_page_id") == page["id"]]
                signature = _generate_layout_signature_for_source(page.get("sample_image_url"), page_fields)
                conn.execute(
                    """
                    INSERT INTO template_pages (
                        id, template_version_id, page_number, page_name, sample_image_url,
                    ...
                    (
                        ...
                        template_id,
                        page_index,
                        page.get("page_name") or f"Page {page_index}",
                        page.get("sample_image_url"),
                        page.get("sample_image_url"),
                        signature_to_json(signature) if signature else None,
                    ),
                )
                ...
                request_page_to_template_page[page["id"]] = mapping
                request_page_number_to_template_page[int(page["page_number"])] = mapping

            converted_requested_fields = [
                field for field in requested_fields
                if field.get("template_request_page_id") in request_page_to_template_page
                or int(field.get("page_number") or 0) in request_page_number_to_template_page
            ]
            for sort_index, field in enumerate(converted_requested_fields):
        ...
        return {
            ...
            "created_records": {
                ...
                "template_versions": 1,
                "template_pages": len(template_pages_source),
                "extraction_fields": len(converted_requested_fields),
            },
        }
